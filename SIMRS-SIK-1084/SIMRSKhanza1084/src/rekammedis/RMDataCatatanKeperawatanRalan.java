...
public final class RMDataCatatanKeperawatanRalan extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public RMDataCatatanKeperawatanRalan(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        tabMode=new DefaultTableModel(null,new Object[]{
            "No.Rawat","No.R.M.","Nama Pasien","Umur","JK","Tgl.Lahir","Tanggal","Jam","Uraian","NIP","Nama Petugas"
        }){
        ...
        Uraian.setDocument(new batasInput((int)2000).getKata(Uraian));
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Catatan Keperawatan Rawat Jalan ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        if(tabMode.getRowCount()==0){
            ...
            if(TCari.getText().trim().equals("")){
                Valid.MyReportqry("rptDataCatatanKeperawatanRalan.jasper","report","::[ Data Catatan Keperawatan Rawat Jalan ]::",
                    ...
                    "catatan_keperawatan_ralan.tanggal between '"+Valid.SetTgl(DTPCari1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(DTPCari2.getSelectedItem()+"")+"' order by catatan_keperawatan_ralan.tanggal,catatan_keperawatan_ralan.jam",param);
            }else{
                Valid.MyReportqry("rptDataCatatanKeperawatanRalan.jasper","report","::[ Data Catatan Keperawatan Rawat Jalan ]::",
                    "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,reg_periksa.umurdaftar,reg_periksa.sttsumur,"+
                    "pasien.jk,pasien.tgl_lahir,catatan_keperawatan_ralan.tanggal,catatan_keperawatan_ralan.jam,catatan_keperawatan_ralan.uraian,"+
                    ...
                    "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                    "inner join petugas on catatan_keperawatan_ralan.nip=petugas.nip where "+
                    "catatan_keperawatan_ralan.tanggal between '"+Valid.SetTgl(DTPCari1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(DTPCari2.getSelectedItem()+"")+"' and "+
                    "(reg_periksa.no_rawat like '%"+TCari.getText().trim()+"%' or pasien.no_rkm_medis like '%"+TCari.getText().trim()+"%' or "+
                    "pasien.nm_pasien like '%"+TCari.getText().trim()+"%' or catatan_keperawatan_ralan.nip like '%"+TCari.getText().trim()+"%' or petugas.nama like '%"+TCari.getText().trim()+"%') "+
                    "order by catatan_keperawatan_ralan.tanggal,catatan_keperawatan_ralan.jam ",param);
            }  
        }
    ...
    private void MnCatatanKeperawatanRawatJalanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCatatanKeperawatanRawatJalanActionPerformed
        if(tbObat.getSelectedRow()>-1){
            ...
            Valid.MyReportqry("rptFormulirCatatanKeperawatanRalan.jasper","report","::[ Formulir Catatan Keperawatan Rawat Jalan ]::",
                    ...
                    "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                    "inner join petugas on catatan_keperawatan_ralan.nip=petugas.nip where reg_periksa.no_rawat='"+tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()+"' order by catatan_keperawatan_ralan.tanggal,catatan_keperawatan_ralan.jam",param);
        }
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMDataCatatanKeperawatanRalan dialog = new RMDataCatatanKeperawatanRalan(new javax.swing.JFrame(), true);
        ...
        try{
            if(TCari.getText().trim().equals("")){
                ps=koneksi.prepareStatement(
                    ...
                    "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                    "inner join petugas on catatan_keperawatan_ralan.nip=petugas.nip where "+
                    "catatan_keperawatan_ralan.tanggal between ? and ? order by catatan_keperawatan_ralan.tanggal,catatan_keperawatan_ralan.jam ");
            }else{
                ps=koneksi.prepareStatement(
                    "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,reg_periksa.umurdaftar,reg_periksa.sttsumur,"+
                    "pasien.jk,pasien.tgl_lahir,catatan_keperawatan_ralan.tanggal,catatan_keperawatan_ralan.jam,catatan_keperawatan_ralan.uraian,"+
                    ...
                    "order by catatan_keperawatan_ralan.tanggal,catatan_keperawatan_ralan.jam ");
            }
    ...
    private void getData() {
        if(tbObat.getSelectedRow()!= -1){
            ...
            Uraian.setText(tbObat.getValueAt(tbObat.getSelectedRow(),8).toString());
    ...
    public void isCek(){
        ...
        BtnEdit.setEnabled(akses.getcatatan_keperawatan_ralan());
        BtnPrint.setEnabled(akses.getcatatan_keperawatan_ralan()); 
        if(akses.getjml2()>=1){
            NIP.setEditable(false);
    ...
    private void ganti() {
        if(Sequel.mengedittf("catatan_keperawatan_ralan","tanggal=? and jam=? and no_rawat=?","no_rawat=?,tanggal=?,jam=?,uraian=?,nip=?",8,new String[]{
            TNoRw.getText(),Valid.SetTgl(Tanggal.getSelectedItem()+""),Jam.getSelectedItem()+":"+Menit.getSelectedItem()+":"+Detik.getSelectedItem(),
            Uraian.getText(),NIP.getText(),tbObat.getValueAt(tbObat.getSelectedRow(),6).toString(),tbObat.getValueAt(tbObat.getSelectedRow(),7).toString(),
            ...
            tbObat.setValueAt(Uraian.getText(),tbObat.getSelectedRow(),8);
            tbObat.setValueAt(NIP.getText(),tbObat.getSelectedRow(),9);
            tbObat.setValueAt(NamaPetugas.getText(),tbObat.getSelectedRow(),10);
            emptTeks();
        }
    }

    private void hapus() {
        if(Sequel.queryu2tf("delete from catatan_keperawatan_ralan where tanggal=? and jam=? and no_rawat=?",3,new String[]{
