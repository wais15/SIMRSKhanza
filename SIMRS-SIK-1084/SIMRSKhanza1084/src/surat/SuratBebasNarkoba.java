...
public final class SuratBebasNarkoba extends javax.swing.JDialog {
   ...
   private void autoSKBN() {
        ...
        int bulan = TanggalSurat.getDate().getMonth() + 1; // getMonth() dimulai dari 0
        String bulanRomawi = getBulanRomawi(bulan);
        String tahun = TanggalSurat.getSelectedItem().toString().substring(6, 10); // Ambil tahun dari tanggal
        
        String sql = "SELECT IFNULL(MAX(CONVERT(LEFT(no_surat,3),SIGNED)),0) " +
                     "FROM surat_skbn " +
                     "WHERE MONTH(tanggalsurat) = '" + bulan + "' AND YEAR(tanggalsurat) = '" + tahun + "'";

        Valid.autoNomerMCU(sql, "/RSUD/MJ/" + bulanRomawi + "/" + tahun, 3, NoSurat);
...
//                "SKBN"+TanggalSurat.getSelectedItem().toString().substring(6,10)+TanggalSurat.getSelectedItem().toString().substring(3,5)+TanggalSurat.getSelectedItem().toString().substring(0,2),3,NoSurat); 
    ...
    return romawi[bulan - 1]; // karena indeks array mulai dari 0
}
}
