    ...
    <div class="entry">        
        <form name="frm_aturadmin" onsubmit="return validasiIsi();" method="post" action="" enctype=multipart/form-data>
	        ...
	        <tr class="head">
                ...
                <tr class="head">
                    <td width="31%" >File Gambar ECHO</td><td width="">:</td>
                    <td width="67%">
                        ...
                        if(($_FILES['gambar']['type'] == 'image/jpeg')||($_FILES['gambar']['type'] == 'image/jpg')){
                            if((@mime_content_type($_FILES['gambar']['tmp_name'])== 'image/jpeg')||(@mime_content_type($_FILES['gambar']['tmp_name'])== 'image/jpg')){
                                if ((!empty($no_rawat))&&(!empty($gambar))) {
                                    if(Tambah(" hasil_pemeriksaan_echo_gambar "," '$no_rawat','$gambar'", " Gambar ECHO " )){
            ...
            <?php
                $_sql = "SELECT * from hasil_pemeriksaan_echo_gambar where hasil_pemeriksaan_echo_gambar.no_rawat='$no_rawat'";
                ...
                if(mysqli_num_rows($hasil)!=0) {
                    echo "<table width='99.6%' border='0' align='center' cellpadding='0' cellspacing='0' class='tbl_form'>
                            <tr class='head'>
                                ...
                                <td width='95%'><div align='center'>Gambar ECHO</div></td>
                            </tr>";
        ...
        <?php
            if ($action=="HAPUS") {
                unlink($_GET['gambar']);
                Hapus(" hasil_pemeriksaan_echo_gambar "," no_rawat ='".validTeks4($_GET['no_rawat'],20)."'","?act=List&action=TAMBAH&no_rawat=$no_rawat");
            }
