...
<div id="post">
    <div class="entry">        
        <form name="frm_aturadmin" onsubmit="return validasiIsi();" method="post" action="" enctype=multipart/form-data>
                        ...
                        if(($_FILES['gambar']['type'] == 'image/jpeg')||($_FILES['gambar']['type'] == 'image/jpg')){
                            if((@mime_content_type($_FILES['gambar']['tmp_name'])== 'image/jpeg')||(@mime_content_type($_FILES['gambar']['tmp_name'])== 'image/jpg')){
                                if ((!empty($no_rawat))&&(!empty($gambar))) {
                                    if(Tambah(" hasil_endoskopi_telinga_gambar "," '$no_rawat','$gambar'", " Gambar Endoskopi " )){
            ...
            <?php
                $_sql = "SELECT * from hasil_endoskopi_telinga_gambar where hasil_endoskopi_telinga_gambar.no_rawat='$no_rawat'";
        ...
        <?php
            if ($action=="HAPUS") {
                unlink($_GET['gambar']);
                Hapus(" hasil_endoskopi_telinga_gambar "," no_rawat ='".validTeks4($_GET['no_rawat'],20)."'","?act=List&action=TAMBAH&no_rawat=$no_rawat");
            }
