        ...
        <form name="frm_aturadmin" onsubmit="return validasiIsi();" method="post" action="" enctype=multipart/form-data>
            ...
            <?php
                $_sql      = "SELECT pegawai.id,pegawai.nik,pegawai.nama,pegawai.departemen,sum(tindakan.jmlh),sum(tindakan.jm) FROM tindakan right OUTER JOIN pegawai ON tindakan.id=pegawai.id and tindakan.tgl like '%".$tahun."-".$bulan."%'
                              where pegawai.stts_aktif='AKTIF' and pegawai.jbtn like '%dokter spesialis%' and (pegawai.nik like '%".$keyword."%' or pegawai.nama like '%".$keyword."%' or pegawai.departemen like '%".$keyword."%') group by pegawai.id order by pegawai.id ASC ";
                $hasil     = bukaquery($_sql);
