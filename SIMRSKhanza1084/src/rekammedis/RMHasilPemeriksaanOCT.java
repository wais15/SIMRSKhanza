 ...
 * @author perpustakaan
 */
public final class RMHasilPemeriksaanOCT extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        MnPenilaianMedis.setText("Formulir Hasil Pemeriksaan OCT");
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Hasil Pemeriksaan OCT ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        Tanggal.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "10-06-2025 20:10:36" }));
        ...
        TabRawat.addTab("Input Hasil Pemeriksaan OCT", internalFrame2);
        ...
        TabData.addTab("Gambar Pemeriksaan OCT", FormPhoto);
        ...
        TabRawat.addTab("Data Hasil Pemeriksaan OCT", internalFrame3);
    ...
    private void MnPenilaianMedisActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnPenilaianMedisActionPerformed
        if(tbObat.getSelectedRow()>-1){
            ...
            Valid.MyReportqry("rptCetakHasilPemeriksaanOCT.jasper","report","::[ Formulir Hasil Pemeriksaan OCT ]::",
                ...
                "inner join dokter on hasil_pemeriksaan_oct.kd_dokter=dokter.kd_dokter where hasil_pemeriksaan_oct.no_rawat='"+tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()+"'",param);
        }
    ...
    private void btnAmbilActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAmbilActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            if(tbObat.getSelectedRow()>-1){
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                Valid.panggilUrl("hasilpemeriksaanoct/login.php?act=login&usere="+koneksiDB.USERHYBRIDWEB()+"&passwordte="+koneksiDB.PASHYBRIDWEB()+"&no_rawat="+tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                this.setCursor(Cursor.getDefaultCursor());
            }else{
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMHasilPemeriksaanOCT dialog = new RMHasilPemeriksaanOCT(new javax.swing.JFrame(), true);
        ...
        try{
            if(TCari.getText().trim().equals("")){
                ps=koneksi.prepareStatement(
                        "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,pasien.tgl_lahir,hasil_pemeriksaan_oct.tanggal,"+
                        "hasil_pemeriksaan_oct.kd_dokter,dokter.nm_dokter,hasil_pemeriksaan_oct.diagnosa_klinis,hasil_pemeriksaan_oct.kiriman_dari,"+
                        "hasil_pemeriksaan_oct.hasil_pemeriksaan "+
                        "from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                        "inner join hasil_pemeriksaan_oct on reg_periksa.no_rawat=hasil_pemeriksaan_oct.no_rawat "+
                        "inner join dokter on hasil_pemeriksaan_oct.kd_dokter=dokter.kd_dokter where "+
                        "hasil_pemeriksaan_oct.tanggal between ? and ? order by hasil_pemeriksaan_oct.tanggal");
            }else{
                ps=koneksi.prepareStatement(
                        "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,pasien.tgl_lahir,hasil_pemeriksaan_oct.tanggal,"+
                        "hasil_pemeriksaan_oct.kd_dokter,dokter.nm_dokter,hasil_pemeriksaan_oct.diagnosa_klinis,hasil_pemeriksaan_oct.kiriman_dari,"+
    ...
    public void isCek(){
        ...
        BtnEdit.setEnabled(akses.gethasil_pemeriksaan_oct());
        BtnEdit.setEnabled(akses.gethasil_pemeriksaan_oct());
        if(akses.getjml2()>=1){
            KdDokter.setEditable(false);
    ...
    private void hapus() {
        if(Sequel.queryu2tf("delete from hasil_pemeriksaan_oct where no_rawat=?",1,new String[]{
    ...
    private void ganti() {
        if(Sequel.mengedittf("hasil_pemeriksaan_oct","no_rawat=?","no_rawat=?,tanggal=?,kd_dokter=?,diagnosa_klinis=?,kiriman_dari=?,hasil_pemeriksaan=?",7,new String[]{
    ...
    private void panggilPhoto() {
        if(FormPhoto.isVisible()==true){
            try {
                ps=koneksi.prepareStatement("select hasil_pemeriksaan_oct_gambar.photo from hasil_pemeriksaan_oct_gambar where hasil_pemeriksaan_oct_gambar.no_rawat=?");
                try {
    ...
    private void simpan() {
        if(Sequel.menyimpantf("hasil_pemeriksaan_oct","?,?,?,?,?,?","No.Rawat",6,new String[]{
                TNoRw.getText(),Valid.SetTgl(Tanggal.getSelectedItem()+"")+" "+Tanggal.getSelectedItem().toString().substring(11,19),KdDokter.getText(),
