	...
	<form name="frm_aturadmin" onsubmit="return validasiIsi();" method="post" action="" enctype=multipart/form-data>
            ...
            <?php
                ...
                while($bariscari = mysqli_fetch_array($hasilcari)) {
                    $_sqlkon        = "SELECT pegawai.stts_kerja FROM pegawai WHERE pegawai.id='$bariscari[0]'";
                    $hasilkon       = bukaquery($_sqlkon);
                    $bariskon       = mysqli_fetch_row($hasilkon);
                    $_sqldansos     = "SELECT dansos.dana FROM dansos";
                    $hasildansos    = bukaquery($_sqldansos);
                    $barisdansos    = mysqli_fetch_row($hasildansos);
                    $dansos         = 0;
                    if($bariskon[0]!="Poc"){
                        $dansos     = isset($barisdansos[0])?$barisdansos[0]:0;
                    }else if($bariskon[0]=="Poc"){
                        $dansos     = 0;
                    }
                    $_sqlpot        = "SELECT koperasi.wajib,jamsostek.biaya,bpjs.biaya from keanggotaan inner join jamsostek on keanggotaan.jamsostek=jamsostek.stts
                                       inner join koperasi on keanggotaan.koperasi=koperasi.stts inner join bpjs on keanggotaan.bpjs=bpjs.stts where keanggotaan.id='$bariscari[0]'";
                    ...
                    if(mysqli_num_rows($hasilpot)!=0) {
                        $_sqlcari2      = "select potongan.id from potongan where potongan.tahun='$tahun' and potongan.bulan='$bulan' and id='$bariscari[0]'";
                        $hasilvalidasi  = bukaquery($_sqlcari2);
                        if(mysqli_num_rows($hasilvalidasi)!=0) {
                            EditData(" potongan ","bpjs='$bpjs',jamsostek='$jamsostek',dansos='$dansos',simwajib='$simwajib' where potongan.tahun='$tahun' and potongan.bulan='$bulan' and id='$bariscari[0]'  ");
                        }elseif(mysqli_num_rows($hasilvalidasi)==0) {
                            InsertData(" potongan ","'$tahun','$bulan','$bariscari[0]','$bpjs','$jamsostek','$dansos','$simwajib','0','0','0','0','0','0','-' ");
                        }
                    }
                }
